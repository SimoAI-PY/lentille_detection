...
def detect_graines(image_path, mode="ellipse"):
    ...
    thresh = cv2.adaptiveThreshold(img_eq, 255, cv2.ADAPTIVE_THRESH_MEAN_C,
    ...
    # Filtrage sur surface (pour ignorer le bruit)
    valid_contours = []
    for cnt in contours:
...
# Interface Streamlit
def main():
    st.set_page_config(page_title="Détection de Graines Saines & Bruchées", page_icon="🌾", layout="wide")
    st.title("🌾 Détection de Graines Saines & Bruchées - U-Net + CNN (Ellipse)")
